...
static int my_fixup(struct phy_device *phydev)
{
	struct lan78xx_net *dev = netdev_priv(phydev->attached_dev);
        //        u32 out;

        pr_info("FIXUP1\n");
        lan78xx_write_reg(dev, 0x128, 0x0);

        phy_write(phydev, 0x17, 0x5a49);
...
static struct phy_device *lan7801_phy_init(struct lan78xx_net *dev)
{
	...
	phydev = phy_find_first(dev->mdiobus);
		...
		if (ret < 0) {
        	...
        	ret = phy_register_fixup_for_uid(0x2000a253, 0xfffffff0,
                	                         my_fixup);
		if (ret < 0) {
                        netdev_err(dev->net, "Failed to register fixup for XXX\n");
                        return NULL;
		}
		...
		if (dev->chipid == ID_REV_CHIP_ID_7801_) {
			if (phy_is_pseudo_fixed_link(phydev)) {
				...
				phy_unregister_fixup_for_uid(PHY_KSZ9031RNX,
							     0xfffffff0);
                                phy_unregister_fixup_for_uid(0x2000a253,
							     0xfffffff0);
...
static void lan78xx_disconnect(struct usb_interface *intf)
{
	...
	phydev = net->phydev;
        ...
        phy_unregister_fixup_for_uid(0x2000a253, 0xfffffff0);
